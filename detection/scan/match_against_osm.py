...
def main() -> int:
    ...
    print(
        f"  confirmed by OSM (<= {DISTANCE_M_CONFIRMED} m): {confirmed_high} ({100 * confirmed_high / max(1, n_high):.0f}%)"
    )
    print(
        f"  new discoveries (>  {DISTANCE_M_CONFIRMED} m): {new_high} ({100 * new_high / max(1, n_high):.0f}%)"
    )
    print()
    ...
    new_high_rows = sorted(
        [r for r in rows if r["tier"] == "high" and r["status"] == "new"], key=lambda r: -r["score"]
    )
    print()
    print("[match] Top 10 NEW high-confidence discoveries (model finds, OSM doesn't have):")
    for r in new_high_rows[:10]:
        print(
            f"  score={r['score']:.3f}  lat={r['lat']:.5f} lon={r['lon']:.5f}  nearest_osm={r['nearest_osm_m']:.0f}m"
        )
